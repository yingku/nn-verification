...
def read_nnet(filename, normalize=False):
    ...
    Returns:
        :class:`~MarabouRepo.maraboupy.MarabouNetworkNNet.MarabouNetworkNNet`
    """
...
def read_tf(filename, inputNames=None, outputNames=None, modelType="frozen", savedModelTags=[]):
    ...
    Returns:
        :class:`~MarabouRepo.maraboupy.MarabouNetworkTF.MarabouNetworkTF`
    """
...
def read_onnx(filename, inputNames=None, outputNames=None):
    ...
    Returns:
        :class:`~MarabouRepo.maraboupy.MarabouNetworkONNX.MarabouNetworkONNX`
    """
...
def load_query(filename):
    ...
    Returns:
        :class:`~MarabouRepo.maraboupy.MarabouCore.InputQuery`
    """
...
def solve_query(ipq, filename="", verbose=True, options=None):
    ...
    Args:
        ipq (:class:`~MarabouRepo.maraboupy.MarabouCore.InputQuery`): InputQuery object, which can be obtained from
                   :func:`~MarabouRepo.maraboupy.MarabouNetwork.getInputQuery` or :func:`~MarabouRepo.maraboupy.Marabou.load_query`
        ...
        options: (:class:`~MarabouRepo.maraboupy.MarabouCore.Options`): Object for specifying Marabou options

    Returns:
        (tuple): tuple containing:
            - exitCode (str): A string representing the exit code (sat/unsat/TIMEOUT/ERROR/UNKNOWN/QUIT_REQUESTED).
            - vals (Dict[int, float]): Empty dictionary if UNSAT, otherwise a dictionary of SATisfying values for variables
            - stats (:class:`~MarabouRepo.maraboupy.MarabouCore.Statistics`, optional): A Statistics object to how Marabou performed
    """
    if options is None:
...
def createOptions(numWorkers=1, initialTimeout=5, initialSplits=0, onlineSplits=2,
    ...
    Returns:
        :class:`~MarabouRepo.maraboupy.MarabouCore.Options`
    """
    options = Options()
