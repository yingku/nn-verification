...
from TL4HDR.data.preProcess import get_one_race, get_n_years, normalize_dataset, get_dataset
from TL4HDR.data.tcga import read_data
from TL4HDR.examples.classify_util import run_mixture_cv, run_one_race_cv, run_CCSA_transfer
...
def run_cv(cancer_type, feature_type, target, years=3):
    ...
    dataset = get_dataset(cancer_type=cancer_type, feature_type=feature_type[0], target=target, groups=("WHITE", "BLACK"))
    ...
    k = 400 if 'mRNA' in feature_type else -1
    ...
    for i in range(20):
        ...
        df_w, _w_classifiers = df_w.rename(columns={"Auc": "W_ind"})
        df_b = run_one_race_cv(seed, dataset_b, **parametrs_b)
        df_b, _b_classifiers = df_b.rename(columns={"Auc": "B_ind"})
